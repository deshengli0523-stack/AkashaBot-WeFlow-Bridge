...
'@
  ...
  Assert-AkashaContains -Text $changelog -Expected '## 0.4.7 - 2026-08-13' -Context 'CHANGELOG.md'
  $version = (Read-AkashaUtf8Strict -Path (Join-Path $root 'VERSION')).Trim()
  if ($version -cne '0.4.7') {
    throw "Documentation/layout gate: VERSION must be 0.4.7, found '$version'."
  }

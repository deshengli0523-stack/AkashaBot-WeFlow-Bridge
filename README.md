...
控制面板的“故障恢复”页会把历史发送提示与仍在生效的阻塞分开显示，并按联系人列出草稿隔离、发送结果未知、未结束 admission、执行中任务和待推送批次。可在这里：

- 只读检查微信主窗口是否可见、最大化并处于前台；如未就绪，请回到微信手动调整窗口；
- 仅作废当前短期能力租约，让合并回复插件安全重新握手，不删除任务或重发消息；
- 检查微信输入框后解除某一条草稿隔离，或根据微信聊天记录精确确认某一条结果未知的发送；
- 先在主面板暂停桥接，再释放某个联系人卡住的发送许可。仍在持久队列中的最新消息会重新取得许可；若该批次早已结束，则让联系人再发一条新消息即可。

草稿隔离和发送结果未知是防止重复发送的数据边界，因此重启不会自动清除。顶部“最近一次发送失败”只是历史提示，可单独关闭；是否仍被阻塞以“故障恢复”页的联系人列表为准。诊断接口不返回消息正文、微信账号、会话 ID、租约或 admission 凭据。

为降低第三方客户端风控风险，控制面板不会主动恢复、显示、置前或最大化微信窗口，也不提供对应写接口。窗口卡片仅作只读诊断；只有用户主动运行的交互式校准器会在校准过程中准备微信窗口。
